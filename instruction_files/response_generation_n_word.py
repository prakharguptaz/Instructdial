...
instruction_dict = {
	"id": "wow",
    "Definitions": ["In this task you will be shown a conversation context. You need to generate a response with N words to the conversation conditioned on the context.",
                    "Provide a response with N words to the conversation",
                    "Generate a response with N words to the conversation",
                    "Generate a response with N words to the dialogue",
                    "Generate a coheent response with N words to the conversation",
                    "Given the conversation, return a response with N words"],
    "Positive Examples": [
        {
    ...
    def generate_data(self):
        ...
        for d, dataset_reader in enumerate(self.data_readers):
            ...
            for dp in tqdm(datapoints):
                ...
                post_prompts = [settings.QUESTION_SEP+" Given this context, the response with [N] number of words is",
                                settings.QUESTION_SEP+" Generate a response with [N] words for the provided context",
                                settings.QUESTION_SEP+" Given this context generate a response with [N] number of words coherent to the context",
                                settings.QUESTION_SEP+" Here is a response with [N] words which is a good follow-up to the context"]
                
                text = text +' '+ random.choice(post_prompts)
                text = text.replace('[N]', str(len(response.split())))
                text = re.sub(' +', ' ', text)
