...
from .multiwoz.dbPointer import *
from difflib import SequenceMatcher

EXPERIMENT_DOMAINS = ["hotel", "restaurant", "attraction", "taxi", "hospital", "police"]
...
def read_langs_turn(args, file_name, ontology, dialog_act, max_line=None, domain_act_flag=False, update_ont_flag=False):
            ...
            for ti, turn in enumerate(dial_dict["dialogue"]):
                ...
                for slot in SLOTS:
                    if slot in belief_dict.keys():
                        ...
                        # update ontology
                        if args["ontology_version"] != "" and "the {}".format(belief_dict[slot]) in ontology[
                            slot].keys():
                ...
                if turn["turn_idx"] == 0 and turn["system_transcript"] == "":
                    ...
                    cur_sys_acts = set()
                elif dialog_act[dial_dict["dialogue_idx"].replace(".json", "")][
                    str(turn["turn_idx"])] == "No Annotation":
                    cur_sys_acts = set()
                else:
                ...
                data_detail["domain"] = turn["domain"]
                data_detail["response"] = data_detail["turn_sys"]
                data_detail["context"] = data_detail["dialog_history"][1:]
                data_detail["acts"] = json.dumps(turn['system_acts'])
                data_detail["act_classes"] = list(data_detail["sys_act"])
                act_classes.update(data_detail["sys_act"])

                belief_str = ' , '.join(['{} : {}'.format(k, v) for k, v in belief_dict.items()])
                data_detail["state"] = belief_str

                #for value in data_detail["del_belief"]:
                #    del data_detail["state"][value]

                #data_detail["state"] = json.dumps(belief_dict)

                db_results = {}
                acts = turn['system_acts']
                    ...
                    for act, values in acts.items():
                        for pair in values:
                            if pair[1] != 'none' and pair[1] != '?':
                                db_results[pair[0]] = pair[1]

                data_detail["db"] = json.dumps(db_results)

                if not args["only_last_turn"] and len(data_detail["response"]) > 0:
    ...
    return data, ontology, response_candidates, act_classes
...
def prepare_data_multiwoz(args):
    ...
    path_to_ontology_mapping = os.path.join(args["data_path"],
                                            "MultiWOZ-{}/ontology-mapping{}.json".format(version,
    ...
    all_act_classes = set([])
    pair_trn, ontology_mapping, resp_cand_trn, act_classes = globals()["read_langs_{}".format(_example_type)](args,
                                                                                                              file_trn,
                                                                                                              ontology_mapping,
                                                                                                              dialog_act,
                                                                                                              max_line,
    ...
    all_act_classes.update(act_classes)
    pair_dev, ontology_mapping, resp_cand_dev, act_classes = globals()["read_langs_{}".format(_example_type)](args,
                                                                                                              file_dev,
                                                                                                              ontology_mapping,
                                                                                                              dialog_act,
                                                                                                              max_line,
                                                                                                              ...
                                                                                                              update_ont_flag)
    all_act_classes.update(act_classes)
    pair_tst, ontology_mapping, resp_cand_tst, act_classes = globals()["read_langs_{}".format(_example_type)](args,
                                                                                                              file_tst,
                                                                                                              ontology_mapping,
                                                                                                              dialog_act,
                                                                                                              max_line,
    ...
    meta_data['act_classes'] = list(all_act_classes)
